...
export class TranscribeTranscriptionService implements TranscriptionService {
  async transcribe(input: { s3Uri: string; mediaFormat: MediaFormat; jobName: string }): Promise<string> {
    ...
    while (Date.now() - start < TRANSCRIBE_WAIT_BUDGET_MS) {
      ...
      if (job?.TranscriptionJobStatus === "COMPLETED") {
        ...
        if (!uri) throw new AiInvocationError("Transcribe job completed but no TranscriptFileUri returned");
        return this.fetchTranscriptText(uri);
      }
  ...
  private async fetchTranscriptText(transcriptFileUri: string): Promise<string> {
    const res = await fetch(transcriptFileUri);
    if (!res.ok) throw new AiInvocationError(`Failed to fetch Transcribe output: HTTP ${res.status}`);
    const json = (await res.json()) as { results?: { transcripts?: Array<{ transcript?: string }> } };
    const text = json.results?.transcripts?.[0]?.transcript;
    if (!text) throw new AiInvocationError("Transcribe output did not contain a transcript");
    return text;
  }
}
